...
function applyForm(data) {
  ...
  els.displayName.value = data.displayName ?? "Esther不二";
  els.handle.value = data.handle ?? "@SjwEsther";
  els.textColor.value = data.textColor ?? "#202938";
  els.accentColor.value = data.accentColor ?? "#2B7FD8";
...
async function handleEditorPaste(event) {
  ...
  if (!files.length) {
    ...
    els.obsidianImportMenu.open = true;
    els.obsidianImportStatus.textContent = `检测到 ${references} 个 Obsidian 图片引用。请先连接 Obsidian 仓库，再重新粘贴这篇文章。`;
...
async function downloadAll() {
  ...
  const zipFilename = state.mode === "scroll" ? "esther-buer-scroll-shot.zip" : "esther-buer-pages.zip";
